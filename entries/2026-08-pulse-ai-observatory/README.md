...
**在线预览**:https://d41c8754b3a14836980c67b0acfcddfa.live-preview.enterapp.pro/(Enter 预览链接,后续可能替换为 prod 正式地址)
...
- **三源雷达**——Hacker News 热榜、GitHub 新晋高星、Lobste.rs;分组展示带来源徽章,同一话题真实出现在多个源时点名「跨源共振」
- **排名动量**——每条携带 ↑n / ↓n / NEW 徽章,服务端与上一份快照对比计算
- **条目一句话点评**——同一次生成调用为全部 25 条条目输出中文点评(仅按标题推断,禁止虚构)
...
3. **六个有界批次,一批一主题**——内容丰富化 → 榜单雷达化(点评+动量)→ 多数据源 → 每周周报 → Agent 工具化深潜。每批 prompt 携带上一轮**已验证**的基线作为不变量,外加明确排除项(明天的范围今天排除)。
...
- 一个数据源(Lobste.rs)在边缘函数网络长期不可达。网站在每轮受影响的解读开头如实注明,用其余源正常交付——降级是设计的一部分,在生产环境真实发生。
