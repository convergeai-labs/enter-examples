...
**Live preview**: https://d41c8754b3a14836980c67b0acfcddfa.prod.enterapp.pro/ (published prod URL)
...
- **Three-source radar** — Hacker News front page, GitHub's new high-star repos, DEV.to daily top; grouped with per-source badges, and cross-source resonance called out when a topic genuinely appears on multiple sources
- **Rank momentum** — every item carries ↑n / ↓n / NEW badges computed server-side against the previous snapshot
- **One-line summaries** — the LLM annotates all 35 items in the same generation pass (title-only inference, no fabrication)
...
3. **Seven bounded batches, one theme each** — content enrichment → radar-ization (summaries + momentum) → multi-source → weekly report → agent tool-use deep-dives → source swap (DEV.to replacing the unreachable Lobste.rs). Each batch prompt carried the previous round's *verified* baseline as its invariants, plus explicit exclusions (tomorrow's scope is excluded today).
...
- The Lobste.rs source was unreachable from the edge function's network: for the first week every affected narrative said so honestly and shipped with the remaining sources (degradation by design, witnessed in production) — then round 7 swapped in DEV.to, and all three sources have been live since.
